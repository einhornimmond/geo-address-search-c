...
static void sql_progress_tick(SqlProgress *sp, const char *table, uint64_t row_count) {
  ...
  printf("\r  SQL-Export: %6.2f%%  %s (%s Zeilen, %s Zeilen/s)  ", pct, table, wr_buf, rt_buf);
  fflush(stdout);
}
...
static void write_copy_section(
  ...
  for (unsigned b = 0; b < PREFIX_BUCKET_COUNT; ++b) {
    ...
    prefix[1] = (char)(b >> 8);
    prefix[2] = '\0';
    size_t prefix_len = prefix[0] ? (prefix[1] ? 2 : 1) : 0;
    ...
    for (ptrdiff_t i = 0; i < hmlen(entries); ++i) {
      ...
      if (e->name) {
        size_t suffix_len = strlen(e->name);
        memcpy(full_name + prefix_len, e->name, suffix_len + 1);
      } else {
...
void storage_stats_write_sql(const StorageStats *stats, const char *filename) {
  ...
  for (int si = 0; si < 7; ++si) {
    for (unsigned b = 0; b < PREFIX_BUCKET_COUNT; ++b) {
      if (all_sets[si]->buckets[b].entries)
        total_rows += (uint64_t)hmlen(all_sets[si]->buckets[b].entries);
    }
  }
