...
grd_result place_cache_seal(const char *directory, const PlaceCacheStamp *stamp) {
  ...
  if (file) {
    ...
    if (fwrite(PLACE_CACHE_MAGIC, 8, 1, file) == 1 &&
        fwrite(stamp, sizeof(*stamp), 1, file) == 1) {
      result = GRD_SUCCESS;
    }
...
static void put_u8(RecordBuilder *b, uint8_t value) { put_bytes(b, &value, 1); }
static void put_i32(RecordBuilder *b, int32_t value) { put_bytes(b, &value, 4); }
static void put_f64(RecordBuilder *b, double value) { put_bytes(b, &value, 8); }
