...
void progress_update(uint64_t currentBytes) {
  ...
  if (elapsed_since_last_call < 200.0) { return; }
  ...
  uint64_t bytesPerSecond = elapsed > 0.0 ? (uint64_t)(currentBytes / elapsed) : 0;

  format_byte_units(speed, sizeof(speed), bytesPerSecond, 2);

  printf(
      "\rProgress: %6.2f%%   %s / %s   (%s/s)", 100.0 * currentBytes / g_totalBytes, current, total,
      speed
  );
