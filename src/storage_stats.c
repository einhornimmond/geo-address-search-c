...
static void key_set_add(
    KeySet *set, const char *const *values, size_t count, const char *name, int has_point
) {
  if (!name) return;
  char key[33];
  ...
  if (index < 0) {
    shputs(
        set->entries, ((Entity){.key = key, .name_bytes = strlen(name), .has_point = has_point})
    );
  } else if (has_point) {
...
void storage_stats_destroy(StorageStats *stats) {
  if (!stats) return;
  shfree(stats->countries.entries);
  shfree(stats->states.entries);
  shfree(stats->counties.entries);
  shfree(stats->cities.entries);
  shfree(stats->streets.entries);
  shfree(stats->houses.entries);
  free(stats);
}
...
void storage_stats_record(StorageStats *stats, yyjson_val *place) {
  ...
  if (street) {
    const char *key[] = {
        country_code, state ? state : "", county ? county : "", city ? city : "", street
    };
  ...
  if (street && yyjson_is_str(house_number)) {
    const char *key[] = {
        country_code, state ? state : "",          county ? county : "", city ? city : "",
        street,       yyjson_get_str(house_number)
    };
...
static void key_set_merge(KeySet *destination, const KeySet *source) {
  for (ptrdiff_t i = 0; i < shlen(source->entries); ++i) {
    ...
    if (found < 0)
      shputs(destination->entries, *entry);
    else if (entry->has_point)
      destination->entries[found].has_point = 1;
  }
}
