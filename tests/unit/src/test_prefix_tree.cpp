...
size_t Intern(PrefixTree *tree, const std::string &name, bool *created = nullptr) {
  ...
  bool made = false;
  EXPECT_EQ(prefix_tree_intern(tree, key, &index, &made), GRD_SUCCESS) << name;
...
protected:
  void SetUp() override { ASSERT_EQ(prefix_tree_init(&tree, 2), GRD_SUCCESS); }
  void TearDown() override { prefix_tree_free(&tree); }
  PrefixTree tree{};
};
...
TEST(PrefixTreeDepth, EveryDepthUpToTheMaximumWorks) {
  for (unsigned depth = 1; depth <= PREFIX_TREE_DEPTH_MAX; ++depth) {
    PrefixTree tree{};
    ASSERT_EQ(prefix_tree_init(&tree, depth), GRD_SUCCESS) << "depth " << depth;
...
TEST(PrefixTreeDepth, DeeperTreesTellApartWhatShallowOnesJoin) {
  ...
  ASSERT_EQ(prefix_tree_init(&shallow, 1), GRD_SUCCESS);
  ASSERT_EQ(prefix_tree_init(&deep, 4), GRD_SUCCESS);
...
TEST(PrefixTreeGuards, RefusesADepthItCannotHold) {
  PrefixTree tree{};
  EXPECT_NE(prefix_tree_init(&tree, 0), GRD_SUCCESS);
  EXPECT_NE(prefix_tree_init(&tree, PREFIX_TREE_DEPTH_MAX + 1), GRD_SUCCESS);
}

TEST(PrefixTreeGuards, NullIsAnsweredRatherThanDereferenced) {
  EXPECT_NE(prefix_tree_init(nullptr, 2), GRD_SUCCESS);
...
TEST(PrefixTreeGuards, FreeingTwiceIsSafe) {
  PrefixTree tree{};
  ASSERT_EQ(prefix_tree_init(&tree, 2), GRD_SUCCESS);
  Intern(&tree, "berlin");
  prefix_tree_free(&tree);
  prefix_tree_free(&tree);
  EXPECT_EQ(prefix_tree_count(&tree), 0u);
